...
dependenciaAdministrativa = cbind(simples = dependenciaAdministrativaSimples,
                                  relativa = dependenciaAdministrativaRelativa)
dependenciaAdministrativa

rendaMensalSimples=table(enem$`Qual é a renda mensal de sua família?`)
rendaMensalRelativa = prop.table(rendaMensalSimples)
rendaMensal = cbind(simples = rendaMensalSimples,
                    relativa = rendaMensalRelativa)
rendaMensal
...
acessoInternet = cbind(simples = acessoInternetSimples,
                       relativa = acessoInternetRelativa)
acessoInternet

linguaEstrangeiraSimples=table(enem$`Língua Estrangeira`)
linguaEstrangeiraRelativa= prop.table(linguaEstrangeiraSimples)
linguaEstrangeira = cbind(simples = linguaEstrangeiraSimples,
                          relativa = linguaEstrangeiraRelativa)
linguaEstrangeira

ensinoMedioSimples=table(enem$`Tipo de escola do Ensino Médio`)
ensinoMedioRelativa= prop.table(ensinoMedioSimples)
ensinoMedio = cbind(simples = ensinoMedioSimples,
                    relativa = ensinoMedioRelativa)
ensinoMedio
